...
export class AuthorsResolver {
  constructor(private authorsService: PodcastsService) {}

  @Query((returns) => Podcast)
  async podcasts(@Args('id', { type: () => Int }) id: number) {
    return this.authorsService.findOneById(id);
  }

  //에러..
  @ResolveField()
  async posts(@Parent() podcasts: Podcast) {
    const { id } = podcasts;
    return this.postsService.findAll({ podcastId: id });
  }
}
/**
